...
class VLLMSampler(SamplerBase):
    ...
    def __init__(
        ...
        if metrics_enabled == "auto":
            # Remote OpenAI-compatible endpoints usually don't expose /metrics.
    ...
    def _get_url_load(self, url: str) -> dict:
        ...
        if not getattr(self, "metrics_enabled", True):
            # For OpenAI-compatible remote endpoints (e.g., DashScope), /metrics is unavailable.
    ...
    def __call__(self, message_list: List[Dict[str, str]]) -> SamplerResponse:
        ...
        while True:
            try:
                ...
                # Small jitter to avoid herd effects when many workers retry together.
